...
class GroupForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if commit:
            groupmachine.save()
        if groupmachine.pk:
            groupmachine.ipmachine_set = self.cleaned_data['ips']
            self.save_m2m()
        return groupmachine
...
class IpAdmin(admin.ModelAdmin):
    ...
    fields = ('hostname', 'ip', 'idc','group',  'port', 'os', 'snmp_on','asset_collection','alert_limit', 'snmp_version', 'snmp_security_level', 'snmp_auth_protocol', 'snmp_user', 'snmp_pass' ,'system_load_warning', 'system_load_critical', 'cpu_idle_warning', 'cpu_idle_critical', 'mem_usage_warning', 'mem_usage_critical')
