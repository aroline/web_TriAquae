...
for i in snmp_monitor_list:
    if i not in mail_dic.keys() and i not in mail_dic['SNMP_ERROR']: # means snmp data not crossing any limit line
        ...
        if ok_ip.snmp_alert_count == 0:pass
        else:
            ok_ip.snmp_alert_count = 0
            ok_ip.save()
            print '\033[32;1mServer performance status back to normal!\033[0m',i
...
#Match the alerting list with relative people
mail_alert_dic = {}
