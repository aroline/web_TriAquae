...
def draw_graph(rrdfile_name,DS,ip,addtional=0):
	if DS == 'CpuUsage':
		...
		--alt-y-grid COMMENT:"Last update %s" ''' % (PIC_DIR,ip,DS,Period_1h, rrdfile_name, rrdfile_name, rrdfile_name,cpu_idle,cpu_system,cpu_user, date)  )
                os.system('''rrdtool graph %s/%s_%s_%s.png \\
                ...
                AREA:value2#0000ff:"cpu_system %s" \\
                AREA:value3#E04000:"cpu_user %s"\\
                --alt-y-grid COMMENT:"Last update %s" ''' % (PIC_DIR,ip,DS,Period_1d, rrdfile_name, rrdfile_name, rrdfile_name,cpu_idle,cpu_system,cpu_user, date)  )
	...
	if DS == 'Load':
		...
		COMMENT:"Last update %s" --alt-y-grid  ''' % (PIC_DIR,ip,DS,Period_1h,rrdfile_name,rrdfile_name,rrdfile_name,load_1,load_5,load_15,date )  )
                os.system('''rrdtool graph %s/%s_%s_%s.png \\
                ...
                LINE2:value2#0000ff:"5min %s" \\
                LINE3:value3#E04000:"15min %s" \\
                COMMENT:"Last update %s" --alt-y-grid  ''' % (PIC_DIR,ip,DS,Period_1d,rrdfile_name,rrdfile_name,rrdfile_name,load_1,load_5,load_15,date )  )
	if DS == 'Mem_list':
		os.system('''rrdtool graph %s/%s_%s_%s.png \\
